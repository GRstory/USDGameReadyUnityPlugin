...
| `gameReady:stepHeight` | float | CharacterController의 `stepOffset` 설정 (기본 컴포넌트일 때만) |
| `gameReady:hasCollider` | token | `"Box"` / `"Sphere"` / `"Capsule"` / `"Mesh"` 중 하나 — 해당 Collider 부착. 단, `isPlayer=true`이고 Player Component가 `(Default)`=CharacterController인 경우 무시됨 |
| `gameReady:colliderSize` | float3 | Collider 또는 NavMeshAgent/CharacterController의 사이즈를 설정. Box=`(x,y,z)`, Sphere=`(radius,_,_)`, Capsule/NavMeshAgent/CharacterController=`(radius,_,height)` |
...
def Xform "World"
{
    ...
    def Xform "Hero"
    {
        ...
        custom float3 gameReady:colliderSize = (50, 0, 180)
        custom float gameReady:slopeAngleLimit = 45.0
        custom float gameReady:stepHeight = 30.0
